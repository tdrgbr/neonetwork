...
const Messages = () => {
  ...
  return (
    <>
      ...
      <div className="ml-7 mt-10 min-lg:ml-90 flex space-x-3 mb-3 font-other text-secondary pr-10">
        <div className="flex flex-col bg-cards w-full h-dvh rounded-xl">
          <div className="flex bg-highlight h-20 p-5 rounded-t-xl justify-start items-center">
            <h1>Messages</h1>
          </div>
          ...
          >
            {conversations.length < 1 ? (
              ...
              conversations.map((msg) => (
                <NavLink
                  key={msg.id}
                  to={`/messages/${msg.id}`}
                  className={`flex items-center space-x-3 min-w-0 p-4 cursor-pointer justify-between transition-all duration-300 border-b-[0.01px] border-profile  ${
                    msg.is_unread
                ...
                >
                  <div className="flex space-x-3">
                    ...
                    <div className="">
                      ...
                      <h6
                        className={`truncate text-ellipsis text-sm text-secondary${
                          msg.is_unread ? "font-semibold" : "font-light"
                        }`}
                      >
                        {msg.last_message}
                      </h6>
            ...
            {hasMore && loading && (
              <div className="flex justify-center py-3 text-secondary">
                <ClipLoader color="" size={25} />
              </div>
            )}
          </div>
        </div>
